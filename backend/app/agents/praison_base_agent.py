"""PraisonAI-based agent implementation for Agent OS V2"""
...
class AgentOSAgent(Agent):
    """Base class for Agent OS V2 agents using PraisonAI"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Build comprehensive instructions for PraisonAI
        ...
        logger.info(f"Initialized Agent OS agent: {name} ({role})")
    
    def _build_agent_instructions(self) -> str:
        """Build comprehensive instructions for the PraisonAI agent"""
        
        instructions = f"""You are {self.name}, a {self.role} in Agent OS V2 - a multi-agent platform for Product Hunt launches and business automation.
...
Remember: You're part of a team helping users launch successful products. Be collaborative, insightful, and action-oriented while maintaining your unique personality and expertise."""
    ...
    async def respond(self, message_content: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """Generate a conversational response using PraisonAI"""
...
Please respond as {self.name} ({self.role}), keeping in mind:
1. Your personality and expertise
2. The conversation history
3. What the user is trying to accomplish
4. How you can best help or what other agents might be needed

Respond naturally and conversationally."""

            # Use PraisonAI's start method to generate response
            response_content = self.start(enhanced_message)
